interface Props { selectedText: string; position: { x: number; y: number }; onSave: (target: 'paper' | 'summary' | 'insight') => void; onQuote: (text: string) => void; onClose: () => void; }

export default function SaveToModulePopup({ selectedText, position, onSave, onQuote, onClose }: Props) {
  ...
  return (
    <div className="fixed z-50 bg-dark-200 rounded-2xl shadow-xl border border-dark-50/30 py-1 min-w-[180px]" style={{ left: position.x, top: position.y }}>
      ...
      <div className="border-t border-dark-50/20">
        <button onClick={() => { onQuote(selectedText); onClose(); }} className="w-full text-left px-3 py-2 text-sm hover:bg-mint-400/10 text-gray-300">Quote to Chat</button>
      </div>
